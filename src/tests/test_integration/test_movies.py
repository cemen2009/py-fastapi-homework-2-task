...
async def test_get_movies_with_custom_parameters(client, seed_database):
    ...
    if page > 1:
        assert response_data["prev_page"] == f"/api/v1/theater/movies/?page={page - 1}&per_page={per_page}", (
            f"Expected prev_page to be 'api/v1/theater/movies/?page={page - 1}&per_page={per_page}', "
    ...
    if page < response_data["total_pages"]:
        assert response_data["next_page"] == f"/api/v1/theater/movies/?page={page + 1}&per_page={per_page}", (
            f"Expected next_page to be '/api/v1/theater/movies/?page={page + 1}&per_page={per_page}', "
